...
- Filtro no topo: minhas / compartilhadas / todas, e uma lupa que abre a busca por nome.
- **A busca vive na URL** (`?q=`), não em estado de componente: compõe com o filtro, sobrevive a recarregar a página e o link pode ser compartilhado. Casa sem acento e sem caixa, com todos os termos em qualquer ordem — a comparação é feita em memória, porque busca sem acento no SQL exigiria a extensão `unaccent`, que existe na Neon mas não no PGlite, e a diferença apareceria só em produção.
